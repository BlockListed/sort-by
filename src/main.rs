use std::{
	io::{self, stderr, stdin, stdout, BufRead, BufWriter, Write},
	process::ExitCode,
};
...
use errors::{argerr_transform, print_error, MainError};
...
fn app(mut args: Arguments) -> Result<(), MainError> {
	...
	let subgroup: usize = args
		.opt_value_from_str("--subgroup")
		.unwrap()
...
fn extract(
	...
	mut extraction: impl FnMut(&str) -> Option<i64>,
) -> Vec<Sortable> {
	lines
		.filter_map(|value| {
